...
const createServer = () => {
  ...
  app.get('/aboutme',(req,res)=>{
    res.status(200).send('My name is Waleed how can I help you!');
  });

  app.get('/favoritefoods',(req,res)=>{
    res.status(200).send(['Everything','Everything']);
  });
...
const count = (target, input) => {
  ...
  return input.reduce((a, row) => {
    return a + row.filter(n => n === target).length;
  }, 0)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(row => {
    return row.filter(n => typeof n === 'number' && n % 5 === 0).map(n => 2 ** n);
  })
};
